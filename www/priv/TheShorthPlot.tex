...
%$Header: /u/math/j40/cvsroot/lectures/src/shorth/Rnw/lshorth.Rnw.tex,v 1.8 2007/09/04 14:58:01 j40 Exp $
...
$$
a,b \mapsto I = [a,b] \mapsto \big( \left| I \right|, P\left(I\right) \big)\qquad \text{where } a \leq b.
$$
...
interval $I$ such that $x \in I$, $\prob{I} = \alpha$ with $|I| = S_{\alpha}(x)$. If $\alpha < 1$, the interval is
...
As a first example, we use the eruption durations of the Old Faithful geyser. The data is just one component of a process data set. 
Looking at a one dimensional marginal distribution ignores the process structure. However these data have been used repeatedly to illustrate smoothing algorithms, and we reuse it to illustrate our approach (see figure \ref{fig:faithful}). This is a good natured data set showing two distict nodes  with sizable observation count, and some overall skewness.
...
Density estimators with varying bandwidth or histograms with varying parameters could reveal these details. The multi scale property of the shorth plot allows to combine the aspects in one pictures.
%
%\clearpage
...
The  chondrite data set was used in to illustrate a strategy to hunt for modes in \cite{Good1980Density-estimat}. See figure \ref{fig:chondrite}. This data set is pressing the limits for the shorth plot since it has a very low sample size with presumably three modes.
...
In this situation, kernel density estimation performs poorly, since it is heavily degraded by boundary effects which cannot cope with the flat parts of the distribution. Note that in this situation, only $25\%$ of the distribution fall in the ``dip'', and thus it must be hidden in higher coverages for the shorth plot.
\begin{figure}[htb]
...
\verb+$Revision: 1.8 $+\\
\verb+$Date: 2007/09/04 14:58:01 $+\\
\verb+$Name:  $+\\
\verb+$Author: j40 $+\\
\begin{verbatim}
$Log: lshorth.Rnw.tex,v $
Revision 1.8  2007/09/04 14:58:01  j40
sync

Revision 1.7  2007/09/02 20:41:51  j40
sync
